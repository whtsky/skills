...
async function queryTickets(from, to, travelDate) {
  // 先检查缓存
  const cachePath = getCachePath(from.station_code, to.station_code, travelDate);
...
function buildMarkdown(tickets, from, to, travelDate, filterDesc) {
  ...
  lines.push('| \u8F66\u6B21 | \u51FA\u53D1\u2192\u5230\u8FBE | \u8017\u65F6 | \u62C5\u5F53 | \u5546\u52A1/\u7279\u7B49 | \u4E00\u7B49\u5EA7 | \u4E8C\u7B49\u5EA7 | \u8F6F\u5367/\u52A8\u5367 | \u786C\u5367 | \u786C\u5EA7 | \u65E0\u5EA7 | \u72B6\u6001 |');
  lines.push('|------|-----------|------|------|-----------|--------|--------|-----------|------|------|------|------|');
  ...
  for (const t of tickets) {
    ...
    lines.push(`| ${t.trainCode} | ${t.departTime}\u2192${t.arriveTime} | ${formatDuration(t.duration)} | ${t.bureau} | ${swz} | ${t.zy} | ${t.ze} | ${rw} | ${t.yw} | ${t.yz} | ${t.wz} | ${buy} |`);
  }
  return lines.join('\n');
}
...
if (!fromStation) { console.error(`未找到车站: ${fromName}`); process.exit(1); }
if (!toStation) { console.error(`未找到车站: ${toName}`); process.exit(1); }

console.error(`查询: ${fromStation.station_name}(${fromStation.station_code}) \u2192 ${toStation.station_name}(${toStation.station_code}) ${date}`);
...
if (values.json) {
  ...
  const outPath = values.output || join(DATA_DIR,
    `${fromStation.station_name}-${toStation.station_name}-${date}.html`);
  mkdirSync(dirname(outPath), { recursive: true });
  writeFileSync(outPath, html);
  console.error(`${filtered.length}/${tickets.length} 趟列车符合条件，已保存到 ${outPath}`);
  console.log(outPath);
}
