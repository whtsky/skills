...
function normalizeDate(d) {
  if (!d) return new Date().toLocaleDateString('sv-SE', { timeZone: 'Asia/Shanghai' });
  // YYYYMMDD → YYYY-MM-DD
...
// 铁路局代码首字母 → 局名
const BUREAU_MAP = {
  B: '哈尔滨局', X: '哈尔滨局', T: '沈阳局', D: '沈阳局',
  L: '沈阳局', P: '北京局', V: '太原局', C: '呼和浩特局',
  F: '郑州局', N: '武汉局', Y: '西安局', K: '济南局',
  H: '上海局', U: '上海局', G: '南昌局', S: '南昌局',
  Q: '广铁', A: '广铁', Z: '南宁局', W: '成都局',
  E: '成都局', M: '昆明局', J: '兰州局', R: '乌鲁木齐局',
  O: '青藏', I: '口岸站',
};
...
function inferBureau(trainNo) {
  ...
  const prefixMap = {
    ...
    '24': '北京局', '2G': '北京局',
    // 上海局
    '5l': '上海局', '55': '上海局', '5q': '上海局', '5i': '上海局',
    // 广铁
    '65': '广铁', '6c': '广铁', '6A': '广铁',
    // 沈阳局
    'T1': '沈阳局', 'T4': '沈阳局', 'D1': '沈阳局',
    // 济南局
    'K1': '济南局', 'K4': '济南局',
    // 成都局
    'W1': '成都局', 'W4': '成都局', 'E1': '成都局',
    // 南昌局
    'G1': '南昌局', 'G4': '南昌局', 'S1': '南昌局',
    // 郑州局
    'F1': '郑州局', 'F4': '郑州局',
    // 武汉局
    'N1': '武汉局', 'N5': '武汉局',
...
async function queryRailGo(trainCodeStr) {
  try {
    const res = await fetch(`https://data.railgo.zenglingkun.cn/api/train/query?train=${encodeURIComponent(trainCodeStr)}`, {
      headers: { 'User-Agent': UA },
...
async function queryEmu(trainCodeStr, targetDate) {
  try {
    const res = await fetch(`https://api.rail.re/train/${encodeURIComponent(trainCodeStr)}`, {
      headers: { 'User-Agent': UA },
      signal: AbortSignal.timeout(5000),
    });
    if (!res.ok) return null;
    ...
    const typeMatch = emuNo.match(/^(CR\d+[A-Z]+|CRH\d+[A-Z]*)/i);
    return { emuNo, trainType: typeMatch?.[1] || emuNo, date: latest.date, note: '非当日数据' };
  } catch {
    return null;
  }
}
...
if (!fromCode || !toCode) {
  console.error(`无法获取站点电报码: ${from_station}(${fromCode || '?'}) / ${to_station}(${toCode || '?'})`);
  process.exit(1);
}
...
if (values.emu || isEmuTrain) {
  ...
  if (emuInfo) {
    console.error(`车底: ${emuInfo.emuNo} (${emuInfo.trainType})${emuInfo.note ? ' [' + emuInfo.note + ']' : ''}`);
  } else {
...
if (emuInfo) console.log(`车底: ${emuInfo.emuNo} (${emuInfo.trainType})${emuInfo.note ? ' [' + emuInfo.note + ']' : ''}`);
if (railgoInfo) {
  const parts = [];
...
const header = padRight('序号', 6) + padRight('站名', 14) + padRight('到达', 10) + padRight('出发', 10) + padRight('停留', 8);
console.log(header);
console.log('----------------------------------------');
